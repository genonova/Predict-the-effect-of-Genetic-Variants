...
inter_train = pickle.load(open('./lstm_res/inter_output_allTrain', 'r'))
inter_train = pd.DataFrame(inter_train)
inter_train.columns = map(lambda x: 'LSTM_'+str(x), inter_train.columns)
train = pd.concat([train, inter_train], axis=1)
print train.shape
y = train['Class'].values
train = train.drop(['Class'], axis=1)
# test (for stage 2)
test = pd.merge(test2, test2x, how='left', on='ID').fillna('')
inter_test = pickle.load(open('./lstm_res/inter_output_allTest', 'r'))
inter_test = pd.DataFrame(inter_test)
inter_test.columns = map(lambda x: 'LSTM_'+str(x), inter_test.columns)
test = pd.concat([test, inter_test], axis=1)
print test.shape
pid = test['ID'].values
# concat train + test
...
pickle.dump(train, open('train2', 'wb'))
pickle.dump(test, open('test2', 'wb'))
pickle.dump(y, open('y2', 'wb'))
